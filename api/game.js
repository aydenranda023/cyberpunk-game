    ...
    try {
        ...
        if (A === 'START_GAME' || A === 'MAKE_MOVE') {
            ...
            if (A === 'MAKE_MOVE') {
                ...
                if (pre) {
                    // Apply Preloaded State
                    let hpChanged = d.hp_change_occurred || false;
                    const preIsChg = !!Object.values(pre.views)[0].stage_1_env; // Trust preload's view of scene change
                    if (preIsChg) hpChanged = false; // Reset flag on scene change
                    ...
                    if (preIsChg) {
                        ...
                        u3.next_scene_change = (curTurn + 1) + 3 + Math.floor(Math.random() * 4); // Set NEXT target
                    }
                    await ref.update(u3);
            ...
            // 2. Generate Content
            // Pass deterministic isChg to AI
            const ai = await run(db, R, U, null, false, isChg);

            // 3. HP Logic Enforcement
            let hpChanged = d.hp_change_occurred || false;
            if (isChg) hpChanged = false; // Reset on new scene

            if (!isChg) {
                Object.values(ai.views).forEach(v => { v.stage_1_env = null; v.location = null; });
            }

            // Guaranteed HP Change Logic
            // If strictly NO change has happened and we are close to next scene change (or random chance), FORCE one.
            // For simplicity: If we are at the turn BEFORE next scene change and no change yet, FORCE it.
            const isLastChance = (curTurn + 1 >= nextChg - 1);
            let forceHp = false;

            if (!hpChanged && !isChg && isLastChance) {
                forceHp = true;
            }

            Object.values(ai.views).forEach(v => {
                if (v.hp_change) {
                    if (hpChanged) v.hp_change = 0; // Already changed, ignore AI
                    else hpChanged = true; // Mark as changed
                } else if (forceHp) {
                    // Force random damage or heal (-10 to +10, bias to damage)
