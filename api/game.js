...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (A === 'START_GAME' || A === 'MAKE_MOVE') {
            ...
            const ai = await run(db, R, U, null, false);
            ...
            const nt = (d.turn || 0) + 1, u3 = { turn: nt };
            if (Object.values(ai.views)[0]?.stage_1_env) u3.last_scene_change = nt;
            await ref.update(u3);
...
async function run(db, rid, uid, sim, isPre) {
    ...
    const pmt = GAME_MASTER_PROMPT.replace('{{HISTORY}}', hist.join("\n")).replace('{{IS_SCENE_CHANGE}}', isChg).replace('{{PLAYER_CONTEXT}}', ctx).replace(/{{PREV_CHOICE}}/g, sim || "上轮");
