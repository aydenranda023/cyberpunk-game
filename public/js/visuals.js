export function initParticles() {
    ...
    const s = new THREE.Scene(), cam = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 1, 1000), r = new THREE.WebGLRenderer({ alpha: true });
    s.fog = new THREE.FogExp2(0, 0.001); cam.position.z = 100; r.setSize(window.innerWidth, window.innerHeight);
    c.innerHTML = ''; c.appendChild(r.domElement);
    const pts = [], cols = [], g = new THREE.BufferGeometry();
    ...
    g.setAttribute('position', new THREE.Float32BufferAttribute(pts, 3)); g.setAttribute('color', new THREE.Float32BufferAttribute(cols, 3));
    const p = new THREE.Points(g, new THREE.PointsMaterial({ size: 2, vertexColors: true, opacity: 0.7, transparent: true })); s.add(p);
...
export async function playDeathSequence() {
    ...
    for (let txt of ["危急...", "FAILURE", "GAME OVER"]) {
